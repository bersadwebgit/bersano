...
import { ChangeSetDto, ChangeSetSchema } from './schemas';
...
export async function generatePlan(opts: PlanOptions): Promise<ChangeSetDto> {
  ...
  const systemPrompt = `You are an elite AI Planner for an e-commerce SaaS platform.
...
Respond STRICTLY with a JSON object matching this schema:
{
  ...
  try {
    ...
    if (result.success) {
      ...
      const validated = ChangeSetSchema.parse(parsed);
      return validated;
    }
