...
export default function NewProductPage() {
  ...
  const [metaSuggestionError, setMetaSuggestionError] = useState('');
  const [creatingSuggestedCategory, setCreatingSuggestedCategory] = useState(false);
  const [creatingSuggestedBrand, setCreatingSuggestedBrand] = useState(false);
  const [categoryPickerKey, setCategoryPickerKey] = useState(0);
  const [brandPickerKey, setBrandPickerKey] = useState(0);
  const [isCategoryApplied, setIsCategoryApplied] = useState(false);
  const [isBrandApplied, setIsBrandApplied] = useState(false);
  ...
  const [variants, setVariants] = useState<{name: string, colorCode: string | null, imageUrl: string | null, price: string, stock: string, isDefault: boolean}[]>([]);
  ...
  const handleGenerateAll = async () => {
    if (!wizardData.productName.trim()) {
      setWizardError('لطفاً ابتدا نام یا ایده کلی محصول را وارد کنید.');
      return;
    }
    setWizardError('');
    setGeneratingAll(true);
    setWizardStatusText('در حال بررسی ایده محصول...');

    const statusSteps = [
      'در حال بررسی ایده محصول و جستجو در منابع...',
      'در حال نگارش مقاله سئو شده و بررسی تخصصی ویژگی‌ها...',
      'در حال استخراج مشخصات فنی و تبلور ویژگی‌های کالا...',
      'در حال طراحی هوشمندانه سوالات متداول مخاطبین...',
      'در حال تنظیم عنوان و توضیحات سئو بهینه و کلیک‌خور...',
    ];
    ...
    const interval = setInterval(() => {
      if (currentStepIdx < statusSteps.length - 1) {
        currentStepIdx++;
        setWizardStatusText(statusSteps[currentStepIdx]);
      }
    }, 4500);
    ...
    while (clientAttempt < maxClientRetries && !success) {
      clientAttempt++;
      if (clientAttempt > 1) {
        setWizardStatusText(`خطایی رخ داد. در حال تلاش مجدد در پس‌زمینه (تلاش ${clientAttempt} از ${maxClientRetries})...`);
        await new Promise(resolve => setTimeout(resolve, 2000));
      }

      try {
        const res = await fetch('/api/admin/products/ai-generate-all', {
          method: 'POST',
        ...
        if (res.ok && data.success) {
          ...
          if (clientAttempt === maxClientRetries) {
            setWizardError(data.error || 'خطایی در تولید کامل محصول رخ داد.');
          }
        }
      } catch (err) {
        if (clientAttempt === maxClientRetries) {
          setWizardError('خطا در برقراری ارتباط با سرور.');
        }
      }
    }
  ...
  const applyGeneratedResult = () => {
    ...
    setFormData(prev => ({
      ...
      brand: generatedResult.brand || prev.brand,
      price: generatedResult.price || prev.price,
      type: wizardData.productType || prev.type,
      categoryId: wizardData.categoryId || prev.categoryId,
      description: generatedResult.description || prev.description,
      fullDescription: generatedResult.fullDescriptionHtml || generatedResult.fullDescription || prev.fullDescription,
      seoTitle: generatedResult.seoTitle || prev.seoTitle,
      seoDescription: generatedResult.seoDescription || prev.seoDescription,
      stock: wizardData.stock || prev.stock,
      wholesalePrice: generatedResult.wholesalePrice || prev.wholesalePrice,
      moq: generatedResult.moq !== undefined ? String(generatedResult.moq) : prev.moq,
      wholesaleUnit: generatedResult.wholesaleUnit || prev.wholesaleUnit,
      wholesaleUnitSize: generatedResult.wholesaleUnitSize !== undefined ? String(generatedResult.wholesaleUnitSize) : prev.wholesaleUnitSize,
      weight: generatedResult.weight !== undefined ? String(generatedResult.weight) : prev.weight,
      volume: generatedResult.volume !== undefined ? String(generatedResult.volume) : prev.volume,
    }));

    if (generatedResult.specsList && Array.isArray(generatedResult.specsList)) {
      setSpecsList(generatedResult.specsList.map((s: any) => ({
        key: s.key || '',
        value: s.value || '',
      })));
    }

    if (generatedResult.featuresList && Array.isArray(generatedResult.featuresList)) {
      setFeaturesList(generatedResult.featuresList.map((f: any) => ({
        key: f.key || '',
        value: f.value || '',
      })));
    }

    if (generatedResult.faqItems && Array.isArray(generatedResult.faqItems)) {
      setFaqItems(generatedResult.faqItems.map((faq: any) => ({
        question: faq.question || '',
        answer: faq.answer || '',
      })));
    }

    if (generatedResult.wholesaleTiers && Array.isArray(generatedResult.wholesaleTiers)) {
      setTiers(generatedResult.wholesaleTiers.map((t: any) => ({
    ...
    if (generatedResult.wholesaleExclusivePrices && Array.isArray(generatedResult.wholesaleExclusivePrices)) {
      setExclusivePrices(generatedResult.wholesaleExclusivePrices.map((ep: any) => ({
        target: ep.target || '',
  ...
  const handleSuggestMeta = async () => {
    if (!formData.title.trim()) {
      setMetaSuggestionError('لطفاً ابتدا نام محصول را وارد کنید.');
      return;
    }

    setSuggestingMeta(true);
    setMetaSuggestionError('');
    setMetaSuggestion(null);
    setIsCategoryApplied(false);
    setIsBrandApplied(false);
    setIsTitleApplied(false);

    try {
      const res = await fetch('/api/admin/products/ai-suggest-meta', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
      ...
      if (!res.ok) {
        throw new Error(data.error || 'خطایی در دریافت پیشنهادات رخ داد.');
      }

      setMetaSuggestion(data);
    } catch (err: any) {
      console.error(err);
      setMetaSuggestionError(err.message || 'ارتباط با سرور برقرار نشد.');
    } finally {
  ...
  const handleApplySuggestedTitle = () => {
    if (!metaSuggestion || !metaSuggestion.suggestedTitle) return;
    setFormData(prev => ({ ...prev, title: metaSuggestion.suggestedTitle || prev.title }));
    setIsTitleApplied(true);
  };

  const handleApplyAllMeta = async () => {
    if (!metaSuggestion) return;

    // Apply Title
    if (metaSuggestion.suggestedTitle) {
      setFormData(prev => ({ ...prev, title: metaSuggestion.suggestedTitle || prev.title }));
      setIsTitleApplied(true);
    }

    // Apply Brand
    if (metaSuggestion.brand) {
      setCreatingSuggestedBrand(true);
      try {
        const brandName = metaSuggestion.brand.trim();
        setFormData(prev => ({ ...prev, brand: brandName }));
        setIsBrandApplied(true);
        ...
        if (res.ok) {
          setBrandPickerKey(prev => prev + 1);
        }
      } catch (e) {
        console.error('Error auto-creating brand on apply all:', e);
      } finally {
        setCreatingSuggestedBrand(false);
      }
    }

    // Apply Category
    if (metaSuggestion.categoryId) {
      setFormData(prev => ({ ...prev, categoryId: metaSuggestion.categoryId || '' }));
      setIsCategoryApplied(true);
    } else if (metaSuggestion.suggestedCategoryName) {
      setCreatingSuggestedCategory(true);
      try {
        const catName = metaSuggestion.suggestedCategoryName.trim();
        ...
        if (res.ok) {
          ...
          setCategoryPickerKey(prev => prev + 1);
          setIsCategoryApplied(true);
        }
      } catch (e) {
        console.error('Error auto-creating category on apply all:', e);
      } finally {
  ...
  const handleApplySuggestedCategory = async () => {
    ...
    try {
      if (metaSuggestion.categoryId) {
        ...
        const data = await res.json();
        if (!res.ok) {
          throw new Error(data.error || 'خطا در ایجاد دسته‌بندی جدید');
        }
      ...
      console.error(err);
      alert(err.message || 'خطا در اعمال دسته‌بندی');
    } finally {
      setCreatingSuggestedCategory(false);
    }
  };

  const handleApplySuggestedBrand = async () => {
    if (!metaSuggestion || !metaSuggestion.brand) return;
    ...
    try {
      ...
      // Try to create the brand in the database if it doesn't exist
      const res = await fetch('/api/admin/brands', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: brandName }),
    ...
    const init = async () => {
      try {
        ...
        if (settingsData.settings) {
          setShopProductType(settingsData.settings.productType || 'both');
          setActivePackage(settingsData.settings.package || null);
          setShopName(settingsData.settings.shopName || 'فروشگاه من');
          setWholesaleEnabled(settingsData.settings.wholesaleEnabled || false);
          const isPackageActive = settingsData.settings.packageExpiresAt ? new Date(settingsData.settings.packageExpiresAt) > new Date() : false;
          if (settingsData.settings.package && isPackageActive) {
            try {
              const features = JSON.parse(settingsData.settings.package.features);
              if (features.maxProducts && features.maxProducts > 0) {
                setProductLimit(parseInt(features.maxProducts));
              }
            } catch (e) {}
          }
          // If the shop only allows digital, default to digital
          if (settingsData.settings.productType === 'digital') {
            setFormData(prev => ({ ...prev, type: 'digital' }));
          }
        }
        ...
        if (productsData.products) {
          setCurrentCount(productsData.products.length);
        }
      } catch (err) {
        console.error(err);
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (productLimit > 0 && currentCount >= productLimit) {
      handleValidationError(`شما به سقف محدودیت پکیج خود (${productLimit} کالا) رسیده‌اید. امکان ثبت محصول جدید ندارید.`);
      return;
    }
      ...
      <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          >
            ...
            <span className="text-[10px] font-normal opacity-80">محصول فقط به صورت عمده فروخته می‌شود و قیمت تک‌فروشی ندارد</span>
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto pb-24">
          ...
          <button
            ...
            onClick={() => {
              ...
              setWizardData({
                productName: '',
                brand: formData.brand || '',
                productType: formData.type || 'physical',
                categoryId: formData.categoryId || '',
                keyHighlights: '',
                targetAudience: '',
                price: formData.price || '',
                ...
                moq: formData.moq || '1',
                wholesaleUnit: formData.wholesaleUnit || 'عدد',
                wholesaleUnitSize: formData.wholesaleUnitSize || '1',
              });
              setGeneratedResult(null);
              setWizardError('');
      ...
      {productLimit > 0 && currentCount >= productLimit && (
        <div className="bg-amber-50 text-amber-800 border border-amber-200 p-5 rounded-2xl mb-6 font-bold text-xs flex flex-col gap-1.5">
          <span className="text-sm">⚠️ محدودیت ظرفیت محصول پکیج فعال است!</span>
          <span>شما در حال حاضر {currentCount} محصول ثبت شده دارید و سقف مجاز پکیج شما {productLimit} کالا می‌باشد.</span>
          <span className="text-[10px] text-amber-600 mt-1">امکان ایجاد کالا جدید مسدود است. لطفاً برای ارتقاء پکیج خود اقدام نمایید.</span>
        </div>
      )}
          ...
          <div className="space-y-4">
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">نام محصول</label>
                {formData.title.trim().length > 0 && (
                  <button
                    type="button"
                    onClick={handleSuggestMeta}
                    disabled={suggestingMeta}
                    className="flex items-center gap-1.5 px-3 py-1 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white rounded-xl text-xs font-bold transition-all shadow-sm hover:shadow-md disabled:opacity-50"
                  >
                    {suggestingMeta ? (
                      <>
                      ...
                      <>
                        ...
                        تکمیل و بهبود با هوش مصنوعی (عنوان، برند، دسته‌بندی)
                      </>
                    )}
                  </button>
                )}
              ...
              {metaSuggestionError && (
                <div className="mt-2 text-xs text-red-600 dark:text-red-400 font-bold">
                  ⚠️ {metaSuggestionError}
                </div>
              )}

              {metaSuggestion && (
                <div className="mt-3 bg-gradient-to-b from-purple-50/70 to-indigo-50/40 dark:from-purple-950/10 dark:to-indigo-950/5 border border-purple-200/80 dark:border-purple-900/30 p-4.5 rounded-2xl text-xs text-purple-900 dark:text-purple-300 space-y-4 shadow-sm">
                  <div className="flex items-start gap-2.5">
                    <Sparkles className="w-5 h-5 text-purple-600 dark:text-purple-400 shrink-0 mt-0.5" />
                    <div>
                      <span className="font-bold block mb-1 text-purple-850 dark:text-purple-200 text-sm">تحلیل هوشمند و حرفه‌ای اطلاعات پایه:</span>
                      <p className="leading-relaxed font-semibold opacity-90">{metaSuggestion.explanation}</p>
                    </div>
                  </div>
                  ...
                  <div className="pt-1.5">
                    <button
                      type="button"
                      onClick={handleApplyAllMeta}
                      className="w-full py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white rounded-xl font-bold text-xs shadow-md transition-all flex items-center justify-center gap-2"
                    >
                      <Check className="w-4 h-4 animate-bounce" />
                      اعمال همه‌جانبه پیشنهادات (عنوان اصلاح‌شده + برند + دسته‌بندی)
                    </button>
                  </div>

                  <div className="flex flex-col gap-3 pt-4 border-t border-purple-200/50 dark:border-purple-900/20">
                    {/* Suggested Title */}
                    {metaSuggestion.suggestedTitle && (
                      <div className="flex items-center justify-between gap-4 bg-white/80 dark:bg-black/20 p-2.5 rounded-xl border border-purple-100/50 dark:border-purple-900/10">
                        <div className="flex flex-col gap-1.5 flex-1 min-w-0">
                          <span className="text-gray-500 dark:text-gray-400 font-bold text-[10px]">عنوان حرفه‌ای و استاندارد پیشنهادی:</span>
                          <span className="font-bold text-purple-800 dark:text-purple-300 bg-purple-100/40 dark:bg-purple-900/20 px-2.5 py-1.5 rounded-lg text-[11px] leading-relaxed block truncate" title={metaSuggestion.suggestedTitle}>
                            {metaSuggestion.suggestedTitle}
                          </span>
                        </div>
                        {isTitleApplied ? (
                          <span className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400 font-bold shrink-0 text-[11px]">
                          ...
                          <button
                            ...
                            className="px-3 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg font-bold text-[10px] transition-colors flex items-center gap-1 shrink-0"
                          >
                            <Check className="w-3.5 h-3.5" />
                            تایید و اصلاح نام کالا
                          </button>
                        )}
                      </div>
                    )}
                    ...
                    {/* Suggested Brand */}
                    {metaSuggestion.brand && (
                      <div className="flex items-center justify-between gap-4 bg-white/80 dark:bg-black/20 p-2.5 rounded-xl border border-purple-100/50 dark:border-purple-900/10">
                        <div className="flex items-center gap-2">
                          <span className="text-gray-500 dark:text-gray-400 font-bold text-[10px]">برند تجاری کالا:</span>
                          <span className="font-bold text-purple-800 dark:text-purple-300 bg-purple-100/40 dark:bg-purple-900/20 px-2.5 py-1 rounded-lg text-[11px]">
                            {metaSuggestion.brand}
                          </span>
                        </div>
                        {isBrandApplied ? (
                          ...
                          <button
                            type="button"
                            onClick={handleApplySuggestedBrand}
                            disabled={creatingSuggestedBrand}
                            className="px-3 py-2 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white rounded-lg font-bold text-[10px] transition-colors flex items-center gap-1 shrink-0"
                          >
                            {creatingSuggestedBrand ? (
                              <Loader2 className="w-3.5 h-3.5 animate-spin" />
                            ) : (
                              <Check className="w-3.5 h-3.5" />
                            )}
                    ...
                    {/* Suggested Category */}
                    {metaSuggestion.categoryId ? (
                      <div className="flex items-center justify-between gap-4 bg-white/80 dark:bg-black/20 p-2.5 rounded-xl border border-purple-100/50 dark:border-purple-900/10">
                        <div className="flex flex-col gap-1 flex-1">
                          <span className="text-gray-500 dark:text-gray-400 font-bold text-[10px]">دسته‌بندی منطبق و شناسایی شده:</span>
                          <span className="font-bold text-purple-850 dark:text-purple-350 bg-purple-100/40 dark:bg-purple-900/20 px-2.5 py-1 rounded-lg text-[11px] self-start">
                            {metaSuggestion.matchedCategoryName || 'بدون نام'}
                          </span>
                        </div>
                        {isCategoryApplied ? (
                          <span className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400 font-bold shrink-0 text-[11px]">
                          ...
                          <button
                            ...
                            disabled={creatingSuggestedCategory}
                            className="px-3 py-2 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white rounded-lg font-bold text-[10px] transition-colors flex items-center gap-1 shrink-0"
                          >
                            {creatingSuggestedCategory ? (
                              <Loader2 className="w-3.5 h-3.5 animate-spin" />
                            ) : (
                              <Check className="w-3.5 h-3.5" />
                            )}
                            تایید و اعمال دسته‌بندی
                          </button>
                        )}
                      </div>
                    ) : metaSuggestion.suggestedCategoryName ? (
                      <div className="flex items-center justify-between gap-4 bg-white/80 dark:bg-black/20 p-2.5 rounded-xl border border-purple-100/50 dark:border-purple-900/10">
                        <div className="flex flex-col gap-1 flex-1">
                          <div className="flex items-center gap-1.5">
                            <span className="text-gray-500 dark:text-gray-400 font-bold text-[10px]">پیشنهاد ایجاد دسته‌بندی جدید:</span>
                            <span className="font-bold text-amber-800 dark:text-amber-400 bg-amber-100/40 dark:bg-amber-900/20 px-2 py-0.5 rounded text-[11px]">{metaSuggestion.suggestedCategoryName}</span>
                          </div>
                          <span className="text-[10px] text-gray-400 dark:text-gray-500 font-medium">این دسته‌بندی در فروشگاه موجود نیست و در صورت تایید جدیداً ایجاد خواهد شد.</span>
                        </div>
                        {isCategoryApplied ? (
                          <span className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400 font-bold shrink-0 text-[11px]">
                            <Check className="w-4 h-4" />
                          ...
                          <button
                            ...
                            onClick={handleApplySuggestedCategory}
                            disabled={creatingSuggestedCategory}
                            className="px-3 py-2 bg-amber-600 hover:bg-amber-700 disabled:opacity-50 text-white rounded-lg font-bold text-[10px] transition-colors flex items-center gap-1 shrink-0"
                          >
                            {creatingSuggestedCategory ? (
                              <Loader2 className="w-3.5 h-3.5 animate-spin" />
                            ) : (
              ...
              {!showSecondaryCategories ? (
                ...
                <div className="space-y-2 border border-dashed border-gray-200 dark:border-gray-800 p-4 rounded-xl relative">
                  <button
                    ...
                    className="absolute top-2 left-2 p-1 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg text-gray-400 hover:text-red-500 transition-colors"
        ...
        <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 space-y-6">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
              ...
              <div className="mt-2 bg-amber-500/5 border border-amber-500/20 rounded-xl p-3 text-[10px] text-amber-700 dark:text-amber-400 font-bold leading-relaxed max-w-2xl">
          ...
          {descMode === 'preview' ? (
            ...
            <div className="space-y-3 animate-in fade-in duration-200">
              <textarea
                ...
                className="w-full px-4 py-3 border border-gray-300 dark:border-gray-700 dark:bg-gray-800 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-gray-900 dark:text-white font-mono text-xs leading-relaxed resize-y"
              />
            </div>
          )}
      ...
      {showAiWizard && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in duration-200" dir="rtl">
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl w-full max-w-xl shadow-2xl overflow-hidden transition-all duration-300">
            {/* Modal Header */}
            <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between bg-gradient-to-r from-purple-500/10 to-indigo-500/10">
              <div className="flex items-center gap-2.5">
                <div className="bg-purple-100 dark:bg-purple-950/40 p-2 rounded-xl text-purple-600 dark:text-purple-400">
                  <Sparkles className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="text-base font-black text-gray-900 dark:text-white">تولید جادویی کل محتوا با هوش مصنوعی</h3>
                  <p className="text-[10px] text-gray-500 dark:text-gray-400 mt-0.5">ثبت یکپارچه مشخصات، توضیحات، سئو و سوالات متداول در چند ثانیه</p>
                </div>
              </div>
              <button 
                type="button" 
            ...
            <div className="p-6">
              ...
              {wizardStep === 1 && !generatedResult && !generatingAll && (
                <div className="bg-blue-50/50 dark:bg-blue-950/20 border border-blue-100/50 dark:border-blue-900/30 p-4 rounded-2xl mb-6 text-xs text-blue-700 dark:text-blue-300 leading-relaxed space-y-1">
                  <p className="font-bold flex items-center gap-1">
                    <HelpCircle className="w-4 h-4 text-blue-500" />
                    راهنمای تولید جادویی هوش مصنوعی
                  </p>
                  <p>
                    این ابزار جادویی به شما کمک می‌کند بدون نیاز به پر کردن دستی فیلدها، تمامی بخش‌های کالا را بر اساس ایده اولیه شما خلق کند. پس از تکمیل فرآیند، کلیدواژه‌ها، سوالات متداول، مقاله تخصصی سئو شده و مشخصات فنی به صورت خودکار بر روی فرم شما اعمال خواهند شد.
                  </p>
                </div>
              )}

              {/* Progress Steps Indicator */}
              {!generatedResult && !generatingAll && (
                <div className="flex items-center justify-between mb-6 text-[10px] font-bold text-gray-400 dark:text-gray-500 border-b border-gray-100 dark:border-gray-800 pb-4">
                  <div className={`flex items-center gap-1 ${wizardStep === 1 ? 'text-purple-600 dark:text-purple-400' : ''}`}>
                    <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] ${wizardStep === 1 ? 'bg-purple-600 text-white' : 'bg-gray-200 dark:bg-gray-800'}`}>۱</span>
                    اطلاعات پایه
                  </div>
                  <div className="w-8 h-px bg-gray-200 dark:bg-gray-800" />
                  <div className={`flex items-center gap-1 ${wizardStep === 2 ? 'text-purple-600 dark:text-purple-400' : ''}`}>
                    <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] ${wizardStep === 2 ? 'bg-purple-600 text-white' : 'bg-gray-200 dark:bg-gray-800'}`}>۲</span>
                    دسته‌بندی و قیمت
                  </div>
                  <div className="w-8 h-px bg-gray-200 dark:bg-gray-800" />
                  <div className={`flex items-center gap-1 ${wizardStep === 3 ? 'text-purple-600 dark:text-purple-400' : ''}`}>
                    <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] ${wizardStep === 3 ? 'bg-purple-600 text-white' : 'bg-gray-200 dark:bg-gray-800'}`}>۳</span>
                    جزئیات کالا
                  </div>
                </div>
              )}
              ...
              {generatingAll ? (
                ...
                <div className="py-8 flex flex-col items-center justify-center text-center space-y-4">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-gray-900 dark:text-white">هوش مصنوعی در حال خلق جادویی کالا...</h4>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 animate-pulse">{wizardStatusText || 'در حال آماده‌سازی اطلاعات...'}</p>
                  </div>
                  <div className="w-full max-w-xs bg-gray-100 dark:bg-gray-800 h-1.5 rounded-full overflow-hidden">
                    <div 
                      className="bg-gradient-to-r from-purple-500 to-indigo-500 h-full rounded-full transition-all duration-500"
                      style={{ 
                        width: wizardStatusText.includes('سئو') ? '100%' : 
                               wizardStatusText.includes('سوالات') ? '80%' :
                               wizardStatusText.includes('مشخصات') ? '60%' :
                               wizardStatusText.includes('مقاله') ? '40%' : '20%' 
                      }}
                    />
                  </div>
                </div>
                ...
                <div className="space-y-4 max-h-[450px] overflow-y-auto pr-1">
                  <div className="bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/40 p-4 rounded-2xl flex items-start gap-3">
                    <div className="bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 p-1.5 rounded-lg mt-0.5">
                      <Check className="w-4 h-4" />
                    </div>
                    <div>
                      <h4 className="text-xs font-bold text-emerald-900 dark:text-emerald-300">محتوای کامل کالا با موفقیت تولید شد!</h4>
                      <p className="text-[10px] text-emerald-700 dark:text-emerald-400 mt-0.5">اطلاعات تولید شده آماده اعمال بر روی فرم محصول شما هستند. خلاصه نتایج زیر را بررسی کنید.</p>
                    </div>
                  </div>

                  <div className="bg-gray-50 dark:bg-gray-800/40 border border-gray-100 dark:border-gray-800 rounded-2xl p-4 text-xs space-y-2.5 font-sans">
                    <div className="flex justify-between border-b border-gray-100 dark:border-gray-800 pb-1.5">
                      <span className="text-gray-500">عنوان محصول:</span>
                      <span className="font-bold text-gray-900 dark:text-white">{generatedResult.title}</span>
                    </div>
                    <div className="flex justify-between border-b border-gray-100 dark:border-gray-800 pb-1.5">
                      <span className="text-gray-500">برند:</span>
                      <span className="font-bold text-gray-900 dark:text-white">{generatedResult.brand || 'ندارد'}</span>
                    </div>
                    <div className="flex justify-between border-b border-gray-100 dark:border-gray-800 pb-1.5">
                      <span className="text-gray-500">قیمت پیشنهادی:</span>
                      <span className="font-bold text-gray-900 dark:text-white">
                        {generatedResult.price ? `${Number(generatedResult.price).toLocaleString('fa-IR')} تومان` : 'نامشخص'}
                      </span>
                    </div>
                    <div className="border-b border-gray-100 dark:border-gray-800 pb-1.5 space-y-1">
                      <span className="text-gray-500 block">توضیح کوتاه:</span>
                      <p className="text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-900 p-2 rounded-lg border border-gray-100 dark:border-gray-800 text-[11px] leading-relaxed">{generatedResult.description}</p>
                    </div>
                    <div className="flex justify-between border-b border-gray-100 dark:border-gray-800 pb-1.5">
                      <span className="text-gray-500">تعداد مشخصات فنی:</span>
                      <span className="font-bold text-purple-600 dark:text-purple-400">{generatedResult.specsList?.length || 0} مورد</span>
                    </div>
                    <div className="flex justify-between border-b border-gray-100 dark:border-gray-800 pb-1.5">
                      <span className="text-gray-500">تعداد ویژگی‌ها:</span>
                      <span className="font-bold text-purple-600 dark:text-purple-400">{generatedResult.featuresList?.length || 0} مورد</span>
                    </div>
                    <div className="flex justify-between border-b border-gray-100 dark:border-gray-800 pb-1.5">
                      <span className="text-gray-500">تعداد سوالات متداول:</span>
                      <span className="font-bold text-purple-600 dark:text-purple-400">{generatedResult.faqItems?.length || 0} مورد</span>
                    </div>
                    <div className="flex justify-between pb-1.5 border-b border-gray-100 dark:border-gray-800">
                      <span className="text-gray-500">عنوان سئو:</span>
                      <span className="font-bold text-blue-600 dark:text-blue-400">{generatedResult.seoTitle}</span>
                    </div>
                    <div className="space-y-1 pb-1.5 border-b border-gray-100 dark:border-gray-800">
                      <span className="text-gray-500 block">توضیحات سئو:</span>
                      <p className="text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-900 p-2 rounded-lg border border-gray-100 dark:border-gray-800 text-[11px] leading-relaxed">{generatedResult.seoDescription}</p>
                    </div>

                    {wholesaleEnabled && generatedResult.wholesalePrice && (
                      <>
                        <div className="flex justify-between border-b border-gray-100 dark:border-gray-800 pb-1.5">
                          <span className="text-gray-500">قیمت عمده‌فروشی پیشنهادی:</span>
                          <span className="font-bold text-emerald-600 dark:text-emerald-400">
                            {Number(generatedResult.wholesalePrice).toLocaleString('fa-IR')} تومان
                          </span>
                        </div>
                        <div className="flex justify-between border-b border-gray-100 dark:border-gray-800 pb-1.5">
                          <span className="text-gray-500">حداقل خرید عمده (MOQ):</span>
                          <span className="font-bold text-gray-900 dark:text-white">
                            {Number(generatedResult.moq || 1).toLocaleString('fa-IR')} {generatedResult.wholesaleUnit || 'عدد'}
                          </span>
                        </div>
                        {generatedResult.wholesaleTiers && generatedResult.wholesaleTiers.length > 0 && (
                          <div className="flex justify-between border-b border-gray-100 dark:border-gray-800 pb-1.5">
                            <span className="text-gray-500">تعداد پله‌های تخفیف عمده:</span>
                            <span className="font-bold text-purple-600 dark:text-purple-400">
                              {generatedResult.wholesaleTiers.length} پله
                            </span>
                          </div>
                        )}
                        {generatedResult.wholesaleExclusivePrices && generatedResult.wholesaleExclusivePrices.length > 0 && (
                          <div className="flex justify-between pb-1.5">
                            <span className="text-gray-500">قیمت‌های اختصاصی همکار/گروه:</span>
                            <span className="font-bold text-purple-600 dark:text-purple-400">
                              {generatedResult.wholesaleExclusivePrices.length} گروه
                            </span>
                          </div>
                        )}
                      </>
                ...
                <div className="space-y-4">
                  {wizardError && (
                    <div className="bg-red-50 dark:bg-red-950/20 text-red-600 dark:text-red-400 border border-red-100 dark:border-red-900 p-3 rounded-xl text-xs font-bold flex items-center gap-1.5 animate-bounce">
                      <AlertCircle className="w-4 h-4 shrink-0" />
                  ...
                  {wizardStep === 1 && (
                    <div className="space-y-4 animate-in fade-in duration-200">
                      <div>
                        <label className="block text-xs font-black text-gray-700 dark:text-gray-300 mb-2">نام یا ایده کلی کالا <span className="text-red-500">*</span></label>
                        <input
                          type="text"
                          required
                          placeholder="مثلاً: گوشی موبایل سامسونگ S24 اولترا یا کفش راحتی مردانه"
                          value={wizardData.productName}
                          onChange={(e) => setWizardData(prev => ({ ...prev, productName: e.target.value }))}
                          className="w-full px-4 py-2.5 text-xs border border-gray-300 dark:border-gray-700 dark:bg-gray-800 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none text-gray-900 dark:text-white"
                        />
                      </div>

                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-xs font-black text-gray-700 dark:text-gray-300 mb-2">برند محصول (اختیاری)</label>
                          <input
                            type="text"
                            placeholder="مثلاً: سامسونگ، شیائومی، نایک"
                            value={wizardData.brand}
                            onChange={(e) => setWizardData(prev => ({ ...prev, brand: e.target.value }))}
                            className="w-full px-4 py-2.5 text-xs border border-gray-300 dark:border-gray-700 dark:bg-gray-800 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none text-gray-900 dark:text-white"
                          />
                        </div>
                        <div>
                          <label className="block text-xs font-black text-gray-700 dark:text-gray-300 mb-2">نوع محصول</label>
                          <select
                            value={wizardData.productType}
                            onChange={(e) => setWizardData(prev => ({ ...prev, productType: e.target.value as 'physical' | 'digital' }))}
                            className="w-full px-4 py-2.5 text-xs border border-gray-300 dark:border-gray-700 dark:bg-gray-800 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none text-gray-900 dark:text-white"
                          >
                            <option value="physical">کالای فیزیکی</option>
                            <option value="digital">کالای دانلودی (فایل)</option>
                          </select>
                        </div>
                      </div>
                    </div>
                  ...
                  {wizardStep === 2 && (
                    <div className="space-y-4 animate-in fade-in duration-200">
                      <div>
                        <label className="block text-xs font-black text-gray-700 dark:text-gray-300 mb-2">انتخاب دسته‌بندی کالا</label>
                        <CategoryPicker 
                          value={wizardData.categoryId} 
                          onChange={(val) => setWizardData(prev => ({ ...prev, categoryId: val }))} 
                        />
                        <span className="text-[10px] text-gray-400 mt-1.5 block">انتخاب دسته‌بندی به هوش مصنوعی در استخراج و غنی‌سازی اطلاعات کمک شایانی می‌کند.</span>
                      </div>

                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-xs font-black text-gray-700 dark:text-gray-300 mb-2">قیمت کالا (تومان - اختیاری)</label>
                          <input
                            type="number"
                            placeholder="مثلاً: 25000000"
                            value={wizardData.price}
                            onChange={(e) => setWizardData(prev => ({ ...prev, price: e.target.value }))}
                            className="w-full px-4 py-2.5 text-xs border border-gray-300 dark:border-gray-700 dark:bg-gray-800 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none text-gray-900 dark:text-white"
                          />
                        </div>
                        <div>
                          <label className="block text-xs font-black text-gray-700 dark:text-gray-300 mb-2">موجودی کالا (اختیاری)</label>
                          <input
                            type="number"
                            placeholder="مثلاً: 10"
                            value={wizardData.stock}
                            onChange={(e) => setWizardData(prev => ({ ...prev, stock: e.target.value }))}
                            className="w-full px-4 py-2.5 text-xs border border-gray-300 dark:border-gray-700 dark:bg-gray-800 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none text-gray-900 dark:text-white"
                          />
                        </div>
                      </div>
                      ...
                      {wholesaleEnabled && (
                        <div className="border-t border-gray-100 dark:border-gray-800 pt-4 mt-4 space-y-4">
                          <p className="text-[11px] font-black text-purple-600 dark:text-purple-400">تنظیمات عمده‌فروشی پیشنهادی (اختیاری)</p>
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <label className="block text-xs font-black text-gray-700 dark:text-gray-300 mb-2">قیمت عمده‌فروشی (تومان)</label>
                              <input
                                type="number"
                                placeholder="مثلاً: 20000000"
                                value={wizardData.wholesalePrice}
                                onChange={(e) => setWizardData(prev => ({ ...prev, wholesalePrice: e.target.value }))}
                                className="w-full px-4 py-2.5 text-xs border border-gray-300 dark:border-gray-700 dark:bg-gray-800 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none text-gray-900 dark:text-white"
                              />
                            </div>
                            <div>
                              <label className="block text-xs font-black text-gray-700 dark:text-gray-300 mb-2">حداقل تعداد سفارش (MOQ)</label>
                              <input
                                type="number"
                                placeholder="مثلاً: 12"
                                value={wizardData.moq}
                                onChange={(e) => setWizardData(prev => ({ ...prev, moq: e.target.value }))}
                                className="w-full px-4 py-2.5 text-xs border border-gray-300 dark:border-gray-700 dark:bg-gray-800 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none text-gray-900 dark:text-white"
                              />
                            </div>
                          </div>
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <label className="block text-xs font-black text-gray-700 dark:text-gray-300 mb-2">واحد عمده</label>
                              <input
                                type="text"
                                placeholder="مثلاً: کارتن، بسته"
                                value={wizardData.wholesaleUnit}
                                onChange={(e) => setWizardData(prev => ({ ...prev, wholesaleUnit: e.target.value }))}
                                className="w-full px-4 py-2.5 text-xs border border-gray-300 dark:border-gray-700 dark:bg-gray-800 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none text-gray-900 dark:text-white"
                              />
                            </div>
                            <div>
                              <label className="block text-xs font-black text-gray-700 dark:text-gray-300 mb-2">تعداد در هر واحد</label>
                              <input
                                type="number"
                                placeholder="مثلاً: 12"
                                value={wizardData.wholesaleUnitSize}
                                onChange={(e) => setWizardData(prev => ({ ...prev, wholesaleUnitSize: e.target.value }))}
                                className="w-full px-4 py-2.5 text-xs border border-gray-300 dark:border-gray-700 dark:bg-gray-800 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none text-gray-900 dark:text-white"
                              />
                            </div>
                          </div>
                  ...
                  {wizardStep === 3 && (
                    <div className="space-y-4 animate-in fade-in duration-200">
                      <div>
                        <label className="block text-xs font-black text-gray-700 dark:text-gray-300 mb-2">ویژگی‌های اصلی، رنگ‌ها یا گارانتی (هر ویژگی در یک خط یا کاما)</label>
                        <textarea
                          rows={3}
                          placeholder="مثلاً: حافظه ۲۵۶ گیگابایت، رنگ آبی اقیانوسی، گارانتی ۱۸ ماهه مدیاپردازش، کیفیت دوربین عکاسی فوق‌العاده"
                          value={wizardData.keyHighlights}
                          onChange={(e) => setWizardData(prev => ({ ...prev, keyHighlights: e.target.value }))}
                          className="w-full px-4 py-2.5 text-xs border border-gray-300 dark:border-gray-700 dark:bg-gray-800 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none text-gray-900 dark:text-white resize-none"
                        />
                      </div>

                      <div>
                        <label className="block text-xs font-black text-gray-700 dark:text-gray-300 mb-2">مخاطبان هدف محصول (اختیاری)</label>
                        <input
                          type="text"
                          placeholder="مثلاً: علاقه‌مندان به عکاسی، گیمرها، ورزشکاران، برنامه‌نویسان"
            ...
            {/* Modal Footer */}
            <div className="p-6 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between bg-gray-50/50 dark:bg-gray-800/10">
              {generatedResult ? (
                <>
                  <button
                    type="button"
                    ...
                    className="px-4 py-2 border border-gray-200 dark:border-gray-800 text-gray-600 dark:text-gray-400 rounded-xl text-xs hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors font-bold cursor-pointer"
                  >
                    تولید مجدد
                  </button>
                  <button
                    type="button"
                    onClick={applyGeneratedResult}
                    className="bg-purple-600 hover:bg-purple-700 text-white px-5 py-2.5 rounded-xl text-xs font-bold flex items-center gap-1.5 cursor-pointer shadow-md transition-colors"
                  >
                    <Check className="w-4 h-4" />
                    اعمال روی محصول و پر کردن فرم
                  </button>
                </>
              ) : generatingAll ? (
                <div className="w-full text-center text-[10px] text-gray-400 dark:text-gray-500 font-bold">
                  صبور باشید؛ فرآیند تولید کامل محتوا ممکن است تا ۳۰ ثانیه زمان ببرد...
                </div>
              ) : (
                <>
                  <button
                  ...
                  <div className="flex gap-2">
                    {wizardStep > 1 && (
                      <button
                        ...
                        className="px-4 py-2 border border-purple-200 dark:border-purple-900/50 text-purple-600 dark:text-purple-400 rounded-xl text-xs hover:bg-purple-50/50 dark:hover:bg-purple-950/10 transition-colors font-bold cursor-pointer"
                      >
                        قبلی
                      </button>
                    )}
                    {wizardStep < 3 ? (
                      <button
                        type="button"
                        onClick={() => {
                          if (wizardStep === 1 && !wizardData.productName.trim()) {
                            setWizardError('لطفاً نام یا ایده کلی محصول را وارد کنید.');
                            return;
                          }
                          setWizardError('');
                          setWizardStep(prev => prev + 1);
                        }}
                        className="bg-purple-600 hover:bg-purple-700 text-white px-5 py-2 rounded-xl text-xs font-bold cursor-pointer transition-colors"
                      >
                        بعدی
                      </button>
                    ) : (
                      <button
                        ...
                        className="bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white px-5 py-2 rounded-xl text-xs font-bold flex items-center gap-1 shadow-md transition-colors cursor-pointer"
                      >
                        <Sparkles className="w-4 h-4" />
                        شروع تولید جادویی
                      </button>
