...
export default function AdminChatPage() {
  ...
  useEffect(() => {
    const handleResize = () => {
      setIsMobile(window.innerWidth < 1024); // lg break point
    };
    handleResize();
  ...
  return (
    <div className="max-w-7xl mx-auto h-[calc(100vh-140px)] flex flex-col select-none font-sans" dir="rtl">
      {/* Top Action Bar */}
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-3 sm:p-4 border border-slate-100 dark:border-slate-800/80 flex justify-between items-center mb-4 shadow-sm">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-2xl bg-blue-50 dark:bg-blue-950/30 text-blue-500 flex items-center justify-center flex-shrink-0">
            <MessageSquare className="w-4 h-4 sm:w-5 sm:h-5" />
          </div>
          <div>
            <h1 className="text-sm sm:text-lg font-black text-slate-800 dark:text-white">مرکز پشتیبانی و چت آنلاین</h1>
            <p className="text-[10px] text-slate-400 hidden sm:block">مدیریت گفتگوهای زنده با مشتریان و تنظیمات پاسخگوی هوشمند</p>
          </div>
        </div>
        <button
          onClick={() => {
          ...
          className="p-2 sm:px-4 sm:py-2 rounded-2xl bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-100 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700/80 transition-all text-xs font-black flex items-center gap-2"
        >
          <Settings className="w-4 h-4" />
          <span className="hidden sm:inline">تنظیمات چت</span>
        </button>
      </div>
      ...
      <div className="flex-1 flex gap-4 overflow-hidden min-h-0 relative">
        ...
        <div className={`w-full lg:w-80 bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800/80 flex-col overflow-hidden shadow-sm ${isMobile && activeSession ? 'hidden' : 'flex'}`}>
          {/* Search & Filters */}
        ...
        <div className={`flex-1 bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800/80 flex-col overflow-hidden shadow-sm ${isMobile && !activeSession ? 'hidden' : 'flex'}`}>
          {activeSession ? (
            <>
              {/* Chat Header */}
              <div className="p-3 sm:p-4 border-b border-slate-100 dark:border-slate-800/80 flex flex-col gap-3 bg-slate-50/40 dark:bg-slate-950/10 shrink-0">
                {/* Upper Row: Back + User Info + CRM Toggle */}
                <div className="flex justify-between items-center w-full">
                  <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                    ...
                    {isMobile && (
                      <button
                        ...
                        className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-850 rounded-xl text-slate-500 transition-colors shrink-0 ml-1"
                    ...
                    {crmProfile?.avatarUrl ? (
                      <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex-shrink-0">
                        <img src={crmProfile.avatarUrl} alt={activeSession.name || 'Avatar'} className="w-full h-full object-cover" />
                      </div>
                    ) : (
                      <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-300 font-bold text-xs sm:text-sm flex-shrink-0">
                        {activeSession.name ? activeSession.name.charAt(0) : 'U'}
                      </div>
                    )}
                    <div className="min-w-0">
                      <h3 className="font-black text-xs sm:text-sm text-slate-800 dark:text-white truncate">
                        {activeSession.name || 'مشتری ناشناس'}
                      </h3>
                      <div className="flex items-center gap-2 mt-0.5 text-[9px] sm:text-[10px] text-slate-400">
                        {activeSession.phone && (
                          <span className="flex items-center gap-0.5">
                            <Phone className="w-2.5 h-2.5 sm:w-3 sm:h-3" /> {activeSession.phone}
                          </span>
                        )}
                        {activeSession.email && !isMobile && (
                          <span className="flex items-center gap-0.5">
                            <Mail className="w-2.5 h-2.5 sm:w-3 sm:h-3" /> {activeSession.email}
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  {/* Actions on Desktop / CRM Toggle on Mobile */}
                    ...
                    {isMobile ? (
                      <button
                        ...
                        className={`p-2 rounded-xl border transition-all flex items-center justify-center ${
                          isCrmOpen 
                            ? 'bg-blue-50 border-blue-100 text-blue-600 dark:bg-blue-950/25 dark:border-blue-900/50 dark:text-blue-400' 
                            : 'bg-slate-50 border-slate-100 text-slate-500 dark:bg-slate-800 dark:border-slate-700'
                        }`}
                        title="اطلاعات CRM خریدار"
                        type="button"
                      >
                ...
                {/* Mobile Only Lower Row for Mode Toggles and Close button */}
                {isMobile && activeSession.status === 'active' && (
                  <div className="flex items-center justify-between gap-2 pt-1 border-t border-slate-100/50 dark:border-slate-800/30 w-full">
                    {/* Compact Mode Toggle */}
                    <div className="flex flex-1 bg-slate-100 dark:bg-slate-800 p-0.5 rounded-xl">
                      <button
                        onClick={() => handleToggleMode('ai')}
                        className={`flex-1 py-1 rounded-lg text-[9px] font-black flex items-center justify-center gap-1 transition-all ${
                          activeSession.mode === 'ai'
                            ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-blue-400 shadow-sm'
                            : 'text-slate-500'
                        }`}
                    ...
                    <button
                      onClick={handleCloseSession}
                      className="px-2.5 py-1.5 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-600 dark:bg-rose-950/20 dark:text-rose-400 transition-all text-[9px] font-black shrink-0"
                    >
                      اتمام گفتگو
                    </button>
                  </div>
                ...
                {loadingDetail ? (
                  ...
                  messages.map(msg => {
                    ...
                    if (msg.messageType === 'product' && msg.metadata) {
                      ...
                      return (
                        <div key={msg.id} className="flex flex-col gap-1.5 max-w-[85%] sm:max-w-[70%] mr-auto">
                    ...
                    return (
                      <div 
                        key={msg.id} 
                        className={`flex flex-col ${isAdmin ? 'items-start' : isAi ? 'items-start' : 'items-end'} max-w-[85%] sm:max-w-[75%] ${isAdmin ? 'mr-auto' : isAi ? 'mr-auto' : 'ml-auto'}`}
                      >
              ...
              {/* Message Input Bar */}
              {activeSession.status === 'active' ? (
                <form onSubmit={handleSendMessage} className="p-2 sm:p-3 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800/80 flex gap-2 items-center shrink-0">
                  <input
                    type="text"
                    value={inputText}
                    onChange={e => setInputText(e.target.value)}
                    placeholder={isMobile ? "پاسخ خود را بنویسید..." : "پاسخ خود را بنویسید... (ارسال پیام پاسخگوی هوشمند را متوقف می‌کند)"}
        ...
        {activeSession && (
          <>
            ...
            {isMobile && isCrmOpen && (
              <div 
                className="fixed inset-0 z-30 bg-black/40 backdrop-blur-[1px] lg:hidden animate-in fade-in duration-200"
                onClick={() => setIsCrmOpen(false)}
              />
            )}
            
            <div className={`
              bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800/80 flex flex-col overflow-hidden shadow-sm p-4 space-y-4 transition-all duration-300
              ${isMobile 
                ? `fixed inset-y-0 left-0 z-40 w-76 sm:w-80 rounded-r-3xl rounded-l-none border-l-0 shadow-2xl transform ${isCrmOpen ? 'translate-x-0' : '-translate-x-full'}` 
                : 'w-80 rounded-3xl'
              }
            `}>
              <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800/80 pb-2">
                <h2 className="text-xs font-black text-slate-800 dark:text-white">اطلاعات خریدار (CRM)</h2>
                {isMobile && (
                  <button 
                    onClick={() => setIsCrmOpen(false)}
