...
5. تنوع‌ها (Variants) - موتور پردازشگر فوق‌العاده هوشمند و همه‌منظوره (Universal Variant Parser - فوق‌العاده حیاتی):
   ...
   - تفاوت اساسی ویژگی‌ها از تنوع‌ها:
     * ویژگی‌ها و مشخصات فنی (featuresList & specsList) صرفاً توصیفی هستند و قیمت یا موجودی جداگانه ندارند.
     * تنوع‌ها (variants) گزینه‌های قابل خرید کالا (مانند رنگ، سایز، حافظه) هستند که هر کدام دارای قیمت، موجودی، شناسه کالا (sku) و ساختار گزینه‌ها (optionsJson) به صورت مجزا می‌باشند.
   
   - ساختار هر تنوع در آرایه variants:
     { id?, name, colorCode, imageUrl, price, stock, isDefault, sku, optionsJson }
     * فیلد optionsJson: یک رشته JSON معتبر (Stringified JSON) است که کلید و مقدار گزینه‌های این تنوع را ذخیره می‌کند. به عنوان مثال، برای تنوع "قرمز - L" باید مقدار آن به صورت '{"رنگ": "قرمز", "سایز": "L"}' باشد. برای تنوع تک‌سطحی "سبز" باید به صورت '{"رنگ": "سبز"}' باشد.
     * فیلد sku: شناسه کالا برای این تنوع است (مثلاً "SH-101-RED-L").
...

{
  ...
  "variants": [
    { "id": "...", "name": "...", "colorCode": "...", "imageUrl": "...", "price": "...", "stock": "...", "sku": "...", "optionsJson": "..." }
  ],
  "faqItems": [
    { "question": "...", "answer": "..." }
  ]
