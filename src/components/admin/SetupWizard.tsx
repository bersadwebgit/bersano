...
export default function SetupWizard({ 
  ...
  const aiSteps = [
    ...
    'تنظیمات هوش مصنوعی با موفقیت در هسته فروشگاه اعمال شد! 🎉'
  ];
  ...
  const submitWizardData = async () => {
    try {
      ...
      if (response.ok) {
        ...
        alert(err.error || 'خطایی در پیکربندی رخ داد.');
        setLoading(false);
        setAiStatusStep(0);
      }
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md select-none" dir="rtl">
      ...
      <div className="relative w-full max-w-2xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden transition-all duration-300">
        ...
        {!loading ? (
          ...
          /* Step 4: Full Glowing AI Processing Simulation Screen */
