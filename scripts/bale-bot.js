...
const LOCK_PORT = 12345;
const pidFile = path.join(__dirname, 'bale-bot.pid');
...
/**
 * Robust Graphical Centralized Long-Polling Bale Bot Runner
 * This script runs locally to poll updates from Bale API using the central_bale_bot_token.
 * It directly interacts with the SQLite database via Prisma to ensure high speed and zero delay.
 * Includes elegant, minimal inline fields for phone and token registration.
 */
...
// Helper to send message to Bale with keyboard
async function sendBaleMessage(chatId, text, replyMarkup = GRAPHICAL_KEYBOARD) {
  try {
...
// Helper to edit message text and inline keyboard in Bale
async function editBaleMessage(chatId, messageId, text, replyMarkup = null) {
  try {
    const url = `https://tapi.bale.ai/bot${botToken}/editMessageText`;
    const response = await fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
async function answerCallbackQuery(callbackQueryId) {
  try {
    const url = `https://tapi.bale.ai/bot${botToken}/answerCallbackQuery`;
    await fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
async function handleStart(chatId) {
  ...
  const welcomeText = `🤖 *به دستیار ربات بله خوش آمدید!*
...
async function sendInlineConnectionPanel(chatId, messageId = null) {
  ...
  if (messageId) {
    const edited = await editBaleMessage(chatId, messageId, text, inlineKeyboard);
    if (!edited) {
      await sendBaleMessage(chatId, text, inlineKeyboard);
    }
  } else {
    await sendBaleMessage(chatId, text, inlineKeyboard);
  }
  ...
  try {
    ...
    if (!shopSettings) {
      ...
      shopSettings = allShops.find(s => {
        if (!s.contactPhone || !s.baleIntegrationToken) return false;
        const normalizedDBPhone = normalizePhone(s.contactPhone);
        return normalizedDBPhone === normalizedInputPhone && s.baleIntegrationToken.trim() === token.trim();
      });
    ...
    console.log(`[SUCCESS] Store "${shopSettings.shopName}" linked to ChatID: ${chatId}`);
    return true;
  } catch (err) {
...
async function handleOrders(chatId) {
  try {
    ...
    if (!shopSettings) {
      await sendBaleMessage(chatId, `⚠️ ابتدا از دکمه «📥 اتصال به فروشگاه» استفاده کنید.`);
      return;
    }
    ...
    if (orders.length === 0) {
      await sendBaleMessage(chatId, `🏪 فروشگاه: *${shopSettings.shopName}*\n📦 سفارشی ثبت نشده است.`);
      return;
    }
...
وضعیت: ${statusText}\n──────────────────\n`;
    ...
    await sendBaleMessage(chatId, orderListText);
  } catch (err) {
    console.error(`[ERROR] Database error during orders fetch:`, err.message);
    await sendBaleMessage(chatId, `❌ خطای سیستمی در دریافت سفارشات.`);
  }
}
...
async function handleDisconnect(chatId) {
  try {
    ...
    if (!shopSettings) {
      await sendBaleMessage(chatId, `⚠️ اکانت شما به هیچ فروشگاهی متصل نیست.`);
      return;
    }
    ...
    console.log(`[INFO] Unlinked ChatID: ${chatId} from store "${shopSettings.shopName}"`);

    await sendBaleMessage(chatId, `❌ *قطع اتصال موفقیت‌آمیز!*

ارتباط اکانت بله شما با فروشگاه *${shopSettings.shopName}* قطع شد.`, null);
  } catch (err) {
    console.error(`[ERROR] Database error during disconnect:`, err.message);
    await sendBaleMessage(chatId, `❌ خطای سیستمی در قطع اتصال.`);
  }
}
...
async function processUpdate(update) {
  ...
  if (update.callback_query) {
    ...
    if (data === 'enter_phone') {
      ...
      await sendBaleMessage(chatId, `📱 شماره موبایل خود را وارد کنید:`, null);
    } else if (data === 'enter_token') {
      userSessions[chatId].state = 'AWAITING_TOKEN';
      userSessions[chatId].panelMessageId = messageId;
      await sendBaleMessage(chatId, `🔑 رمز اتصال خود را وارد کنید:`, null);
    } else if (data === 'confirm_link') {
      const session = userSessions[chatId];
      if (!session.phone || !session.token) {
        await sendBaleMessage(chatId, `⚠️ لطفا ابتدا شماره و رمز را وارد کنید!`, null);
        return;
      }
      ...
      if (success) {
        ...
        const shopSettings = await prisma.shopSettings.findFirst({
          where: {
            ...
            baleIntegrationToken: session.token,
          }
        });
        const shopName = shopSettings ? shopSettings.shopName : 'فروشگاه شما';
        await sendBaleMessage(chatId, `🎉 *اتصال با موفقیت برقرار شد!*\n🏪 فروشگاه: *${shopName}*`);
      } else {
  ...
  if (text === '/cancel') {
    delete userSessions[chatId];
    await sendBaleMessage(chatId, `❌ فرآیند لغو شد.`);
    return;
  }
  ...
  if (session && session.state === 'AWAITING_PHONE') {
    ...
    if (normalizedInputPhone.length !== 10 || !normalizedInputPhone.startsWith('9')) {
      await sendBaleMessage(chatId, `❌ شماره موبایل نامعتبر است. لطفاً شماره صحیح ۱۱ رقمی بفرستید:`, null);
      return;
    }
  ...
  // Default Fallback
  await sendBaleMessage(chatId, `⚠️ متوجه دستور شما نشدم. از دکمه‌های زیر چت استفاده کنید.`);
}

// Long-polling loop
async function startPolling() {
  console.log(`\n🤖 central Bale Bot Long-Polling loop started successfully.`);
  console.log(`📡 Checking for updates... (Press Ctrl+C to stop)`);

  while (isPolling) {
    try {
      const url = `https://tapi.bale.ai/bot${botToken}/getUpdates?offset=${offset}&timeout=10`;
      const response = await fetch(url, { signal: AbortSignal.timeout(15000) });
      
      if (!response.ok) {
        console.warn(`[WARN] Bale API returned status ${response.status}. Retrying in 5s...`);
...
async function main() {
  console.log(`=================================================`);
  console.log(`🚀 [BALE BOT RUNNER] Starting central bot service`);
  console.log(`=================================================`);
